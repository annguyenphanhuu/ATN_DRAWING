...
def plot_view_data(ax, view_data, color):
    ...
    for line in view_data['Lines']:
        start_point = line.get('StartPointOrigin')
        end_point = line.get('EndPointOrigin')
...
def create_visualization_for_view(case_id, view_all, view_client, output_path):
    ...
    fig.suptitle(f'Visualization for Case: {case_id} - View: {view_name} (Origin)')

    # Plot DimAll data
    ax1.set_title('DimAll Data (Origin)')
    plot_view_data(ax1, view_all, 'blue')
    ax1.set_aspect('equal', adjustable='box')
    ax1.grid(True)

    # Plot DimClient data
    ax2.set_title('DimClient Data (Origin)')
...
def main():
    ...
    visualization_dir = os.path.join('data', 'visualizations_origin')
